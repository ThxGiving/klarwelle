...
public class TunerUsbImpl implements TunerUsb {
	...
	/**
	 * Called from native (JTunerUsbDevice) for every decoded FIG 0/15 (DAB EWS / ASA) on the tuned
	 * ensemble, including heartbeats. Builds a {@link DabEwsAlert} and fans it out to listeners.
	 */
	public void ewsAlert(int form, boolean otherEnsemble, int idValue, int stage, int incidentId,
						 boolean test, boolean truncated, String description, String[] locationCodes,
						 int tunedEnsembleId) {
		if(DEBUG)Log.d(TAG, "EWS FIG0/15: " + description);
		DabEwsAlert alert = new DabEwsAlert(form, otherEnsemble, idValue, stage, incidentId, test, truncated, description, locationCodes, tunedEnsembleId);
		for(TunerListener cb : mTunerlisteners) {
			cb.tunerDabEwsAlert(this, alert);
		}
	}
	...
	private class SerializeServicesTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... voids) {
			...
			// Hybrid RadioDNS/SPI-over-IP enrichment stripped (DAB-only build).
