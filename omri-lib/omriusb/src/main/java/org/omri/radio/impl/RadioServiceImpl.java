...
public abstract class RadioServiceImpl implements RadioService, Serializable {
	...
	void serviceStopped() {
		...
		if(mAudioDec != null) {
			// Keep the codec alive for the next service — the factory reuses/flushes it (fast switch,
			// no teardown/rebuild race). Just stop delivering this service's audio. It is released on
			// a real format change (factory) or full teardown (stopAll).
			mAudioDec.setCodecCallback(null);
		}
