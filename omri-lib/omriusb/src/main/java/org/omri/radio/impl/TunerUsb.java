...
interface TunerUsb extends Tuner {
	...
	// DAB EWS (ASA) FIG 0/15. MUST be declared here: native resolves the method id on this interface
	// (native-lib FindClass "org/omri/radio/impl/TunerUsb"), so leaving it only on the impl made
	// GetMethodID fail with a pending NoSuchMethodError that crashed omri init.
	void ewsAlert(int form, boolean otherEnsemble, int idValue, int stage, int incidentId,
