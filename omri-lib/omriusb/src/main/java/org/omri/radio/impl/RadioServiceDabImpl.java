...
public class RadioServiceDabImpl extends RadioServiceImpl implements RadioServiceDab, Serializable {
	...
	// Broadcast DAB->FM link (FIG 0/6 + 0/21), set from native at scan time. 0 = none (the norm in
	// Germany, which uses implicit SId==PI linking instead). Read by the app for service following.
	private int mLinkedFmFrequency = 0;   // kHz
	private int mLinkedFmPi = 0;          // RDS PI, 0 = unknown

	void setLinkedFmFrequency(int freqKhz) {
		this.mLinkedFmFrequency = freqKhz;
	}

	void setLinkedFmPi(int pi) {
		this.mLinkedFmPi = pi;
	}

	/** Linked FM frequency in kHz from the broadcast service link, or 0 if none was signalled. */
	public int getLinkedFmFrequency() {
		return mLinkedFmFrequency;
	}

	/** Linked FM RDS PI from the broadcast service link, or 0 if none was signalled. */
	public int getLinkedFmPi() {
		return mLinkedFmPi;
	}
	...
	public boolean equalsRadioService(RadioService otherSrv) {
		if(otherSrv != null) {
			...
			// IP-service equality (RadioDNS bearer match) stripped (DAB-only build).
		}
