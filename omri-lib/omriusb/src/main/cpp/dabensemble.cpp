...
void DabEnsemble::registerCbs() {
    ...
    m_015Ptr = m_ficPtr->registerFig_00_15_Callback(std::bind(&DabEnsemble::fig00_15_input, this, std::placeholders::_1));
...
// Service linking (FIG 0/6) collection — enabled so m_serviceLinkDb populates where broadcast. Note:
// German muxes use implicit SId==PI linking and don't send this, so it stays empty here (the app
// resolves DAB->FM itself via RadioLogic.linkDabToFm). Kept for UK/CH-style explicit-link ensembles.
...
DabEnsemble::LinkedFmInfo DabEnsemble::getLinkedFm(uint16_t serviceId) const {
    LinkedFmInfo result;
    // FIG 0/6: find a linkage set whose key service is this DAB service, carrying RDS PI links.
    for(const auto& entry : m_serviceLinkDb) {
        const Fig_00_Ext_06::ServiceLinkingInformation& sli = entry.second;
        if(static_cast<uint16_t>(sli.keyServiceId & 0xFFFF) != serviceId) {
            continue;
        }
        for(const auto& sll : sli.serviceLinks) {
            if(sll.idListQualifier != Fig_00_Ext_06::SERVICE_LINKING_IDENTIFIER_LIST_QUALIFIER::RDS_PI) {
                continue;
            }
            for(uint32_t rawPi : sll.idList) {
                uint16_t pi = static_cast<uint16_t>(rawPi & 0xFFFF);
                // FIG 0/21: the FM frequency for this PI (the freq DB is keyed by the PI as its id).
                auto fiIter = m_frequencyInformationDb.find(pi);
                if(fiIter != m_frequencyInformationDb.cend()) {
                    for(const auto& fi : fiIter->second) {
                        if(fi.frequencyInformationType == Fig_00_Ext_21::FrequencyInformationType::FM_RDS && !fi.frequenciesKHz.empty()) {
                            result.valid = true;
                            result.pi = pi;
                            result.freqKHz = fi.frequenciesKHz.front();
                            return result;                    // best case: PI + real FM frequency
                        }
                    }
                }
                // We have the linked PI but no frequency yet — still useful, the app can match by PI.
                if(!result.valid) {
                    result.valid = true;
                    result.pi = pi;
                    result.freqKHz = 0;
...
void DabEnsemble::fig00_15_input(const Fig_00_Ext_15& fig15) {
    // FIG 0/15 EWS. Forward every decoded instance (heartbeat + alert) to listeners; the receiver-
    // side policy (matching, playback) lives above this library.
    std::cout << m_logTag << " " << fig15.description() << std::endl;
    m_ewsAlertDispatcher.invoke(fig15);
}
...
std::shared_ptr<DabEnsemble::Ews_Alert_Callback> DabEnsemble::registerEwsAlertCallback(DabEnsemble::Ews_Alert_Callback cb) {
    return m_ewsAlertDispatcher.add(cb);
}
