...
RaonTunerInput::RaonTunerInput(std::shared_ptr<JTunerUsbDevice> usbDevice) : m_usbDevice{usbDevice} {
    ...
    m_ewsAlertCb = DabEnsemble::registerEwsAlertCallback(std::bind(&RaonTunerInput::onEwsAlert, this, std::placeholders::_1));
}

void RaonTunerInput::onEwsAlert(const Fig_00_Ext_15& alert) {
    // FIG 0/15 arrives on whatever ensemble is currently tuned. Forward to the Java tuner; the
    // receiver-side EWS policy lives in the app.
    if(m_usbDevice != nullptr) {
        m_usbDevice->ewsAlert(alert, getEnsembleId());   // EId of the ensemble we're tuned to
    }
}
