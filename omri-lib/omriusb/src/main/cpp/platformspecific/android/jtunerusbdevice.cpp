...
void JTunerUsbDevice::setJavaClassUsbTuner(JNIEnv* env, jclass usbTunerClass) {
    ...
    m_usbTunerEwsAlertMId = env->GetMethodID(m_usbTunerClass, "ewsAlert", "(IZIIIZZLjava/lang/String;[Ljava/lang/String;I)V");
    // Never let a resolution failure leave a pending exception that would corrupt the JNI calls below.
    if(env->ExceptionCheck()) { env->ExceptionClear(); m_usbTunerEwsAlertMId = nullptr; }
}
...
void JTunerUsbDevice::setJavaClassDabService(JNIEnv* env, jclass dabServiceClass) {
    ...
    m_dabServiceSetLinkedFmFrequencyMId = env->GetMethodID(m_dabServiceClass, "setLinkedFmFrequency", "(I)V");
    m_dabServiceSetLinkedFmPiMId = env->GetMethodID(m_dabServiceClass, "setLinkedFmPi", "(I)V");
...
void JTunerUsbDevice::ensembleReady(const DabEnsemble& ensemble) {
    ...
    for(const auto& srv : ensemble.getDabServices()) {
        ...
        // Broadcast DAB->FM link (FIG 0/6 SId->PI joined with FIG 0/21 PI->FM frequency). Empty in
        // Germany (implicit SId==PI linking, not broadcast) — carried through opportunistically.
        DabEnsemble::LinkedFmInfo fmLink = ensemble.getLinkedFm(static_cast<uint16_t>(srv->getServiceId() & 0xFFFF));
        if(fmLink.valid) {
            enve->CallVoidMethod(dabServiceObject, m_dabServiceSetLinkedFmFrequencyMId, (jint)fmLink.freqKHz);
            enve->CallVoidMethod(dabServiceObject, m_dabServiceSetLinkedFmPiMId, (jint)fmLink.pi);
        }
...
void JTunerUsbDevice::ewsAlert(const Fig_00_Ext_15& alert, uint16_t tunedEnsembleId) {
    // Defensive: never call with an unresolved method id (would be undefined behaviour). If the Java
    // signature ever fails to resolve, skip silently rather than crash omri.
    if(m_usbTunerEwsAlertMId == nullptr) {
        return;
    }
    bool wasDetached = false;
    JNIEnv* enve;
    ...
    const bool oe = alert.isOtherEnsembleAlert();
    const jint form = static_cast<jint>(alert.getForm());
    const jint idValue = oe ? static_cast<jint>(alert.getEnsembleId())
                            : static_cast<jint>(alert.getSubChId());
    const jint stage = alert.hasStatus() ? static_cast<jint>(alert.getStage()) : -1;
    const jint incident = alert.hasStatus() ? static_cast<jint>(alert.getIncidentId()) : -1;

    jstring jdesc = enve->NewStringUTF(alert.description().c_str());

    // Location codes as "zone:digithex" strings (e.g. "1:92c"), the alert area for §7.5.4 matching.
    const auto& locs = alert.getLocationCodes();
    jclass strClass = enve->FindClass("java/lang/String");
    jobjectArray jlocs = enve->NewObjectArray(static_cast<jsize>(locs.size()), strClass, nullptr);
    for(size_t i = 0; i < locs.size(); ++i) {
        std::ostringstream os;
        os << static_cast<int>(locs[i].zone) << ':' << std::hex << static_cast<int>(locs[i].digit1);
        for(uint8_t d : locs[i].otherDigits) {
            os << std::hex << static_cast<int>(d);
        }
        jstring js = enve->NewStringUTF(os.str().c_str());
        enve->SetObjectArrayElement(jlocs, static_cast<jsize>(i), js);
        enve->DeleteLocalRef(js);
    }

    enve->CallVoidMethod(m_usbTunerObject, m_usbTunerEwsAlertMId,
                         form, (jboolean)oe, idValue, stage, incident,
                         (jboolean)alert.isTest(), (jboolean)alert.isTruncated(), jdesc, jlocs,
                         (jint)tunedEnsembleId);
    if(jdesc != nullptr) {
        enve->DeleteLocalRef(jdesc);
    }
    enve->DeleteLocalRef(jlocs);
    enve->DeleteLocalRef(strClass);
