...
void FicParser::call(const std::vector<uint8_t> &data) {
    ...
    while (ficIter < data.end()) {
        ...
        if(FIB_CRC_CHECK(fib.data())) {
            ...
            // Weak reception corrupts a great many FIBs (tens of thousands per session) — logging
            // every one buried the log (and cost a log write each time). Count them and report
            // periodically instead; the running total still shows how bad reception is.
            static unsigned long s_fibCrcErrors = 0;
            if ((++s_fibCrcErrors % 500UL) == 0UL) {
                std::cout << M_LOG_TAG << " FIB crc corrupted x" << s_fibCrcErrors << " (weak reception)" << std::endl;
            }
        }
...
void FicParser::parseFig_00(const std::vector<uint8_t>& ficData) {
    ...
    switch (ficData[0] & 0x1F) {
        ...
        case Fig::FIG_00_TYPE::EMERGENCY_WARNING_SYSTEM: {
            // FIG 0/15 — DAB Emergency Warning System (ETSI TS 104 089). Decode + fan out; no state
            // is held in the parser (the alert database only exists during an alert).
            Fig_00_Ext_15 ext15(ficData);
            m_fig00_15dispatcher.invoke(ext15);
            break;
        }
...
std::shared_ptr<FicParser::Fig_00_15_Callback> FicParser::registerFig_00_15_Callback(Fig_00_15_Callback cb) {
    return m_fig00_15dispatcher.add(cb);
}
