...
public:
    ...
    /** Broadcast DAB->FM link for a service, resolved from the FIG 0/6 (SId->PI) + FIG 0/21 (PI->FM
     *  frequency) databases. valid=false when the ensemble carries no such link (the norm in DE). */
    struct LinkedFmInfo {
        bool valid{false};
        uint32_t freqKHz{0};
        uint16_t pi{0};
    };
    LinkedFmInfo getLinkedFm(uint16_t serviceId) const;
    ...
    // FIG 0/15 — DAB Emergency Warning System (ETSI TS 104 089). Surfaced like date/time (a clean
    // dispatcher), NOT like announcements (whose input dead-ends without reaching listeners).
    using Ews_Alert_Callback = std::function<void(const Fig_00_Ext_15&)>;
    virtual std::shared_ptr<DabEnsemble::Ews_Alert_Callback> registerEwsAlertCallback(Ews_Alert_Callback cb);
...
private:
    ...
    void fig00_15_input(const Fig_00_Ext_15& fig15);
...
private:
    ...
    std::shared_ptr<std::function<void (const Fig_00_Ext_15&)>> m_015Ptr;
...
private:
    ...
    CallbackDispatcher<Ews_Alert_Callback> m_ewsAlertDispatcher;
